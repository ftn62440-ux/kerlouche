...
module.exports =
{
    ...
    */
    async execute(client, message, args)
    {
        ...
        if (!clients[member.id]?.token) return message.channel.send(`\`${member.displayName}\` (${member}) n'est pas connecté à la machine`);
        
        loadSelfbot(clients[member.id].token);
        clients[member.id].destroy();
        delete clients[member.id];
    ...
    */
    async executeSlash(client, interaction)
    {
        ...
        if (!clients[member.id]?.token) return interaction.reply({ content: `\`${member.displayName}\` (${member}) n'est pas connecté à la machine`, flags: 64 });
