...
module.exports = 
{
    ...
    */
    async execute(client, message, args) 
    {
        ...
        if (!res?.id) return message.channel.send(`Le token est invalide ou n'est pas un token d'utilisateur`);

        if (clients[res.id]) return message.channel.send(`\`${res.global_name ?? res.username}\` (<@${res.id}>) est déjà connecté`);
        loadSelfbot(args[0]);
    ...
    */
    async executeSlash(client, interaction) 
    {
        ...
        if (!res?.id) return interaction.editReply(`Le token est invalide ou n'est pas un token d'utilisateur`);

        if (clients[res.id]) return interaction.editReply(`\`${res.global_name ?? res.username}\` (<@${res.id}>) est déjà connecté`);
        loadSelfbot(token);
