...
module.exports = {
    ...
    run: async (client, message, args) => {
        ...
        if (client.relationships.blockedCache.has(user.id)) return message.edit(`***\`${user.displayName}\` est déjà bloqué***`);

        await client.api.users('@me').relationships(user.id).put({ data: { type: 2 } })
        message.edit(`***\`${user.displayName}\` a été bloqué***`)
    }
};