...
module.exports = {
    ...
    */
    run: async (client, message, args) => {
        ...
        if (!role.editable) return message.edit(`***Je ne peux pas ajouter le rôle \`${role.name}\`***`);

        const members = message.guild.members.cache.filter(m => !m.roles.cache.has(role.id) && (args[0] === 'all' || (args[0] === 'bot' ? m.user.bot : !m.user.bot)));
        if (members.size === 0) return message.edit(`***Aucun membre à qui ajouter le rôle \`${role.name}\`***`);
        ...
        const m = await message.channel.send(`Je suis en train d'ajouter le rôle à **${members.size}** membres\nJ'ai ajouté le rôle à **${noe}** membres\nJe n'ai pas pu ajouter le rôle à **${err}** membres`)
        const int = setInterval(() => m.edit(`Je suis en train d'ajouter le rôle à **${members.size}** membres\nJ'ai ajouté le rôle à **${noe}** membres\nJe n'ai pas pu ajouter le rôle à **${err}** membres`), 5000);
        ...
        m.edit(`Je suis en train d'ajouter le rôle à **${members.size}** membres\nJ'ai ajouté le rôle à **${noe}** membres\nJe n'ai pas pu ajouter le rôle à **${err}** membres`);
        clearInterval(int)
    }
};