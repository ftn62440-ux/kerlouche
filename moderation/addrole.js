...
module.exports = {
    ...
    */
    run: async (client, message, args) => {
        ...
        if (!member.manageable) return message.edit(`***Je n'ai pas la permission de donner un rôle à \`${member.user.displayName}\`***`);
        if (message.member.roles.highest.position <= role.position) return message.edit(`***Je n'ai pas la permission de donner ce rôle***`);
        
        member.roles.add(role)
            .then( () => message.edit(`***Le rôle \`${role.name}\` a été ajouté à \`${member.user.displayName}\`***`))
            .catch(() => message.edit(`***Je n'ai pas pu donner le rôle \`${role.name}\` à \`${member.user.displayName}\`***`));
    }
};